...
# Creates an array that stores the paths for each cutoff i
# as in: [cutoffi[ [paths_fragment], [paths_fragment], .. ], cutoffn[paths for fragments, ..]]
def ceate_path_structure(path):

    # Creating a list containing the min amount of 
    # patches for a given cutoff and a list for each cutoff i
...
def save_lists(cutoff_list):
    ...
    for i in range(0, 21, 1):
        ...
        np.save('/projects/mdhali/BscProjects/Stephan/Unsorted_paths/Cutoff_15/Arrays/Paths_' + ext[i] + '.npy' , cutoff_list[i], allow_pickle = True)
